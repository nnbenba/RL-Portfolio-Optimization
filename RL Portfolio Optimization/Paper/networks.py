...
# ── Dimensions (must match test.py) ──────────────────────────────────────────
N_SECTORS = 9           # after dropping XLC and XLRE (incomplete history)
N_ASSETS  = N_SECTORS + 1  # 9 sectors + cash
T         = 60          # lookback window in days
INPUT_DIM = N_ASSETS * T   # flattened state matrix: 10 × 60 = 600
...
if __name__ == "__main__":
    ...
    # Dummy batch: 4 state matrices, all-cash initial portfolio
    states = torch.zeros(4, N_ASSETS, T)
    states[:, -1, 0] = 1.0  # cash weight = 1
