...
from environment import PortfolioEnv, N_ASSETS, LOOKBACK, STATE_DIM, N_REGIMES
...
TURNOVER_COEF   = 0.002    # lambda = c: cost per unit turnover in R_net = R_t - lambda*Sum|dw|
                          # (= 20 bps). Set 0.0 to recover the plain frictionless DSR.
REB_FREQ        = 5        # rebalance every 5 trading days (weekly); agent still acts daily
...
def _make_env(data, belief=None, ns_model=None, seed=None):
    """Build an env. `belief` is the per-window (D, N_REGIMES) HMM posterior; if
    None a uniform belief is used (fine for date-index lookups only). `ns_model`
    is the per-window non-stationary return generator that supplies r_hat_{t+1}."""
...
def collect_rollouts(envs, env_states, policy, critic, device, train_start, train_end):
    ...
    """
    n = len(envs)
    states_b  = np.zeros((N_STEPS, n, STATE_DIM), dtype=np.float32)   # (756, 10, 554)
...
def ppo_update(policy, critic, optimizer, rollout, advantages, returns, device, lr_frac):
    ...
    """
    states  = torch.tensor(rollout["states"].reshape(-1, STATE_DIM), device=device)
...
def evaluate(data, policy, start_idx, end_idx, device, belief=None, ns_model=None):
    """
    Run policy deterministically on [start_idx, end_idx).
    Returns (total_D_t_reward, final_portfolio_value).
    Env RNG is seeded (0) so the appended r_hat_{t+1} draws are reproducible.
    """
    env     = _make_env(data, belief, ns_model, seed=0)
...
def train_window(w_idx, window, data, seed_policy_sd, device):
    ...
    # NSMDP belief b_t: fit the HMM on THIS window's train span only, then filter
    # forward (leak-free). Shared across seeds/envs for the window.
    belief, _ = regime_belief(data, train_start, train_end)
    print(f"  belief fit on train [{train_start}:{train_end}] "
          f"(bear/neu/bull mean over train: "
          f"{belief[train_start:train_end].mean(0).round(3)})")

    # Non-stationary return generator: regime-mixture Student-t fit on the same
    # train span. Supplies the synthetic r_hat_{t+1} appended to vec(R_t).
    R_tr     = data[1].values.astype(np.float64)[train_start:train_end]
    ns_model = NonStationaryReturnModel(n_assets=R_tr.shape[1]).fit(
        R_tr, belief[train_start:train_end])
    print(f"  ns-model fit: regime dof v_k={ns_model.nu_k.round(1)}")
    ...
    for seed in range(N_SEEDS):
        ...
        # Parallel environments — each starts at a random training date, each with
        # its own seeded RNG so the appended r_hat_{t+1} draws differ across envs.
        envs       = [_make_env(data, belief, ns_model, seed=int(np.random.randint(2**31)))
        ...
        val_r, val_pv = evaluate(data, policy, val_start, val_end, device, belief, ns_model)
